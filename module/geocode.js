...
const resultHandler = (result = {}, err) => {
  return err ? { status: "fail", error: err } : { status: "success", result };
};
...
async function geocode(address, city = "") {
  ...
  const [regoErr, reGeoJson] = await handle(
    api.get(getRegeoUrl(addressObj.location))
  );
  ...
  if (
    reGeoJson.status !== "1" ||
    !(reGeoJson.regeocode && reGeoJson.regeocode["addressComponent"])
  ) {
    return;
  }
